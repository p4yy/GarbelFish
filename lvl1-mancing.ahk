...
While (status=="Start"){
	while (fishing=="shiny")
	{
		...
		pCaptha := Gdip_CreateBitmapFromFile("image-shiny\captha.PNG")
		...
		while (status=="Start")
		{
			...
			startCaptha := Gdip_ImageSearch(pGrowtopiaScreen,pCaptha,LISTCAPTHA,0,0,0,0,0,0xFFFFFF,1,0)
			If startCaptha = 1
			{
				ControlSend,,{escape},ahk_pid %Window%
			}
			Else If startWater = 1
			{
			...
			Else If (startCaptha = 0) And (startWater = 0) And (startTutorial = 0)
			{   
				ControlClick,,ahk_pid %Window%,,Left,1,X%waterX% Y%WaterY% NA 
				Sleep,2500
				ControlClick,,ahk_pid %Window%,,Left,1,X%waterX% Y%WaterY% NA  
	...
	while (fishing=="deto")
	{
		...
		hwnd := WinExist("ahk_exe Growtopia.exe") 
		;hwnd := WinExist("Payy 1") using wintitle but need different wintitle
		pCaptha := Gdip_CreateBitmapFromFile("image-deto\captha.PNG")
		...
		while (status=="Start")
		{
			...
			Else If (startCaptha = 0) And (startWaterDeto = 0) And (startDeto = 0) And (startTutorial = 0)
			{   
				ControlClick,,ahk_pid %Window%,,Left,1,X%waterX% Y%WaterY% NA 
				Sleep,2500
				ControlClick,,ahk_pid %Window%,,Left,1,X%waterX% Y%WaterY% NA 
